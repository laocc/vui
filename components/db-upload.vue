<template>
  <div class="form" :style="cssBtn">
    ...
    <a v-show="!(run && progress)" :style="cssH" class="btnUP fc f40a" :class="cls" :href="href" @click="upload"
       onclick="return !1;">
      <slot></slot>
    </a>
...
module.exports = {
      ...
      if (url instanceof Array) {
        url.forEach((pam, i) => {
          if (typeof pam === 'object') {
            url[i] = encodeURI(JSON.stringify(pam));
          }
        });
        url = url.shift().sprintf(...url);
      }
      return url;
    },
  },
  ...
  methods: {
    ...
    uploadNow() {
      ...
      self.http.open("POST", self.href, true); //post方式，url为服务器请求地址，true 该参数规定请求是否异步处理。
      ...
      self.http.upload.onloadstart = function (proEnt) {//上传开始执行方法
        console.log('upload.onLoadStart', self.href, proEnt);
